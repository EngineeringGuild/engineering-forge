...
import { Lesson, Course, ILesson, ICourse } from '../models/Lesson';
...
export class DatabaseService {
   ...
   */
  public async findProjectsByUser(userId: string, limit: number = 10, skip: number = 0): Promise<IProject[]> {
    try {
   ...
   */
  public async findPublicProjects(limit: number = 10, skip: number = 0): Promise<IProject[]> {
    try {
   ...
   * Update project
   */
  public async updateProject(id: string, updateData: Partial<IProject>): Promise<IProject | null> {
    try {
   ...
   */
  public async findPublishedCourses(limit: number = 10, skip: number = 0): Promise<ICourse[]> {
    try {
      const courses = await Course.find({ isPublished: true })
        .sort({ 'enrollment.averageRating': -1, 'enrollment.totalEnrolled': -1 })
        .limit(limit)
        .skip(skip);
      return courses;
    } catch (error) {
  ...
  public async getStatistics(): Promise<{
    ...
    try {
      const [users, projects, lessons, courses, publicProjects, publishedCourses] = await Promise.all([
        User.countDocuments(),
