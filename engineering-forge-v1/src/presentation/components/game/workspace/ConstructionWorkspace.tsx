...
export const ConstructionWorkspace: React.FC<ConstructionWorkspaceProps> = ({
  ...
  const handleMouseDown = useCallback(
    (event: React.MouseEvent, component: Component) => {
      ...
      const offset = new PositionVO(
        mouseX - component.position.x,
  ...
  const handleMouseMove = useCallback(
    (event: React.MouseEvent) => {
      ...
      const newPosition = new PositionVO(
        mouseX - dragOffset.x,
        mouseY - dragOffset.y
      );
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 pointer-events-none">
        <div className="absolute top-4 left-4 text-xs text-blue-400/60 font-medium">
          Drop Zone
        </div>
      ...
      {components.length > 1 && (
        ...
        >
          {components.map((component, index) => {
            ...
            const startX =
              prevComponent.position.x + prevComponent.size.width / 2;
            const startY =
              prevComponent.position.y + prevComponent.size.height / 2;
