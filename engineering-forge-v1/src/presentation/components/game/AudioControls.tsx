import { MousePointer, Music, Settings, Volume2, VolumeX, Zap } from 'lucide-react';
import React, { useCallback, useEffect, useState } from 'react';
import { AudioService, AudioSettings } from '../../../domains/gaming/services/AudioService';
    ...
    return (
      <div className={`flex items-center space-x-2 ${className}`}>
        <AnimatedButton
          ...
          size="sm"
          onClick={toggleMute}
          icon={settings.muted ? <VolumeX className="w-4 h-4" /> : <Volume2 className="w-4 h-4" />}
    ...
    <GlassCard variant="default" className={`p-4 ${className}`}>
      ...
      <div className="space-y-4">
        ...
        <div>
          <div className="flex items-center justify-between mb-2">
            ...
            <div className="flex items-center space-x-2">
              <span className="text-sm text-gray-400">{Math.round(settings.sfxVolume * 100)}%</span>
              <AnimatedButton
                variant="ghost"
                size="sm"
        ...
        <div>
          <div className="flex items-center justify-between mb-2">
            ...
            <div className="flex items-center space-x-2">
              <span className="text-sm text-gray-400">{Math.round(settings.uiVolume * 100)}%</span>
              <AnimatedButton
                variant="ghost"
                size="sm"
      ...
      <div className="mt-4 pt-4 border-t border-white/10">
        <div className="flex items-center justify-between text-sm">
          ...
          <span className="text-white font-medium">{audioService.getPlayingSounds().length}</span>
        </div>
      </div>
    </GlassCard>
  );
