...
export const AchievementNotification: React.FC<AchievementNotificationProps> = ({
  ...
  return (
    ...
    >
      <div
        className={`bg-gradient-to-r ${getRarityGradient(achievement.rarity)} rounded-lg p-4 shadow-lg ${getRarityGlow(achievement.rarity)} border border-white/20 min-w-80 max-w-96`}
      >
        {/* Header */}
        <div className="flex items-start justify-between mb-3">
          <div className="flex items-center">
            <div className="text-3xl mr-3 animate-bounce">{achievement.icon}</div>
            <div>
        ...
        <div className="text-xs text-white/60 mt-2 text-center">
          Auto-closes in {Math.ceil((autoCloseDelay - progress * (autoCloseDelay / 100)) / 1000)}s
        </div>
      </div>
    </div>
  );
