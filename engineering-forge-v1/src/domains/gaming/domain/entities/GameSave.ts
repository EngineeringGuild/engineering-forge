 ...
 * /Users/user/Desktop/Core Guild Project/projects/Games/Engineering Forge/engineering-forge-v1/src/domains/gaming/domain/entities/GameSave.ts
 *
...
export class GameSave {
  ...
  static createNew(
    ...
    const now = new Date();
    const id = `save_${userId}_${Date.now()}_${Math.random()
      .toString(36)
      .substr(2, 9)}`;
  ...
  exportToFile(): void {
    ...
    link.href = url;
    link.download = `${this.data.saveName.replace(/[^a-z0-9]/gi, "_")}_${
      this.data.version
    }.json`;
