...
export class ProjectService {
  ...
  async getPublicProjects(options: {
    ...
    if (options.type) {
      filteredProjects = filteredProjects.filter(
        (p) => p.type === options.type
      );
    }

    if (options.difficulty) {
      filteredProjects = filteredProjects.filter(
        (p) => p.difficulty === options.difficulty
      );
    }
  ...
  async addComponentToProject(
    ...
    project.performance = await this.calculateProjectPerformance(
      project.components
    );
   ...
   */
  private async calculateProjectPerformance(
    componentIds: string[]
  ...
  async getProjectStatistics(): Promise<{
    ...
    const totalScore = allProjects.reduce(
      (sum, p) => sum + p.performance.score,
      0
    );
