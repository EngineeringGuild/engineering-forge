...
export const COMPONENT_DATA: Array<{
  ...
  },
  {
    ...
    category: "mechanical",
    properties: {
      power: 100, // 100 HP - mais realista
      weight: 180,
      efficiency: 60,
      durability: 70,
      cost: 2500,
  ...
  },
  {
    ...
    icon: "🌱",
    description:
      "Advanced hybrid system combining combustion and electric power.",
    level: 1,
  },
  ...
  },
  {
    ...
    icon: "💎",
    description:
      "Ultra-lightweight carbon fiber chassis with exceptional strength.",
    level: 1,
  },
...
export const COMPONENT_TYPES_BY_CATEGORY: Record<
  ComponentCategory,
  ComponentType[]
> = {
...
export function createComponent(
  ...
  const componentProperties = ComponentProperties.create(
...
export function getComponentsByType(type: ComponentType): Component[] {
  return COMPONENT_DATA.filter((component) => component.type === type).map(
...
export function getComponentsByRarity(rarity: ComponentRarity): Component[] {
  return COMPONENT_DATA.filter((component) => component.rarity === rarity).map(
    (component) => createComponent(component)
  );
}
...
export function getUnlockedComponents(userLevel: number): Component[] {
  return COMPONENT_DATA.filter(
    (component) => component.properties.unlockLevel <= userLevel
  ).map((component) => createComponent(component));
}
...
export const DEFAULT_COMPONENT_SIZES: Record<
  ComponentType,
